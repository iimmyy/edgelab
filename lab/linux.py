...
def preflight(minimum_free=2 * 1024**3):
    if os.geteuid() != 0 or sys.platform != "linux":
        raise RuntimeError("requires root on Linux")
    if os.uname().machine != "aarch64":
        raise RuntimeError("requires the ARM64 lab")
    if shutil.disk_usage("/var/lib").free < minimum_free:
    ...
    return {
        ...
        "devices": json.loads(
            run("lsblk", "--json", "-b", "-o", "NAME,SIZE,TYPE,MOUNTPOINTS")
        ),
    }
...
def init():
    ...
    if ROOT.exists():
        guard()
        return facts
    if facts["free_bytes"] < 12 * 1024**3:
        raise RuntimeError("less than 12 GiB free")
    names = run("ip", "netns", "list")
    if any(n in names for n in (CLIENT, SERVER)) or run(
        "vgs", "edgelab_r2", check=False
    ):
...
def storage():
    ...
    if path.exists():
        ...
        if not os.path.ismount(f["mount"]):
            run("mount", f["lv"], f["mount"])
        return f
    if (ROOT / "storage-started").exists():
        raise RuntimeError(
            "interrupted initial provisioning; preserve resources for inspection"
        )
    (ROOT / "storage-started").touch()
    devices = []
    for i in range(4):
        p = ROOT / f"disk{i}"
        run("fallocate", "-l", 1800 * 1024**2, p)
    ...
    lv = "/dev/edgelab_r2/data"
    run("mkfs.ext4", "-q", lv)
    mount = ROOT / "volume"
    mount.mkdir()
    run("mount", lv, mount)
    with (mount / ".owner").open("w") as f:
        f.write("objects-1\n")
        f.flush()
        os.fsync(f.fileno())
    fd = os.open(mount, os.O_DIRECTORY)
    os.fsync(fd)
    os.close(fd)
    result = {
        "machine": machine(),
...
def validate_storage(f):
    if f["machine"] != machine() or len(f["devices"]) != 4:
        raise RuntimeError("invalid storage ownership")
...
def network():
    ...
    if CLIENT in existing and SERVER in existing:
        if not (ROOT / "network-ready").exists():
            baseline()
            (ROOT / "network-ready").touch()
        return
    if CLIENT in existing or SERVER in existing:
        raise RuntimeError("partial network; inspect before repair")
    for name in (CLIENT, SERVER):
        run("ip", "netns", "add", name)
        ns(name, "ip", "link", "set", "lo", "up")
    run("ip", "link", "add", "el-c", "type", "veth", "peer", "name", "el-s")
    run("ip", "link", "set", "el-c", "netns", CLIENT)
    run("ip", "link", "set", "el-s", "netns", SERVER)
    for name, link, address in [
        (CLIENT, "el-c", "172.30.77.1/30"),
        (SERVER, "el-s", "172.30.77.2/30"),
    ]:
        ns(name, "ip", "addr", "add", address, "dev", link)
...
def public(name):
    return run("wg", "pubkey", input=(ROOT / (name + ".key")).read_text())


def baseline():
    for name, address, peer, endpoint in [
        (CLIENT, "10.77.0.1", SERVER, "172.30.77.2"),
        (SERVER, "10.77.0.2", CLIENT, "172.30.77.1"),
    ]:
        ns(name, "ip", "addr", "flush", "dev", "wg0")
        ns(name, "ip", "addr", "add", address + "/32", "dev", "wg0")
    ...
    ns(
        ...
        "-f",
        "-",
        input='table inet edgelab { chain input { type filter hook input priority 0; policy accept; iifname "el-s" tcp dport { 8101, 9000 } counter drop; }; }\n',
    )
...
def alive(name):
    ...
    try:
        ...
        return fields[21] == record["start"] and fields[2] != "Z"
...
def start(name, args, namespace=SERVER):
    ...
    with (ROOT / (name + ".log")).open("ab") as log:
        p = subprocess.Popen(
            ["ip", "netns", "exec", namespace, *map(str, args)],
            stdout=log,
            stderr=log,
    ...
    save(
        name + ".pid",
        {"pid": p.pid, "start": Path(f"/proc/{p.pid}/stat").read_text().split()[21]},
    )
    time.sleep(0.2)
    if p.poll() is not None:
        raise RuntimeError(
            f"{name} failed: {(ROOT / (name + '.log')).read_text()[-2000:]}"
        )


def stop(name):
    if alive(name):
        pid = json.loads((ROOT / (name + ".pid")).read_text())["pid"]
        os.kill(pid, 15)
        for _ in range(60):
            if not alive(name):
                break
...
def workloads():
    ...
    save(
        "proxy.json",
        {"Apps": [{"Name": "objects", "Ports": [8101], "Targets": ["127.0.0.1:9000"]}]},
    )
    start(
        "proxy",
        [
...
def fault(name):
    if name == "interface":
        ns(CLIENT, "ip", "link", "set", "wg0", "down")
    elif name == "address":
        ns(CLIENT, "ip", "addr", "del", "10.77.0.1/32", "dev", "wg0")
        ns(CLIENT, "ip", "addr", "add", "10.77.0.9/32", "dev", "wg0")
        ns(CLIENT, "ip", "route", "replace", "10.77.0.2/32", "dev", "wg0")
    elif name == "endpoint":
        ns(
            CLIENT,
            "wg",
        ...
        ns(CLIENT, "ip", "route", "replace", "blackhole", "10.77.0.2/32")
    elif name == "firewall":
        ns(
            SERVER,
            "nft",
...
def status():
    result = {
        "time": time.time(),
        "machine": machine(),
        "namespaces": run("ip", "netns", "list"),
        "processes": {
            n: json.loads((ROOT / (n + ".pid")).read_text())
            for n in ["objects", "proxy"]
            if (ROOT / (n + ".pid")).exists()
        },
    }
    for n in [CLIENT, SERVER]:
        result[n] = {
            "addresses": ns(n, "ip", "-j", "addr"),
            "routes": ns(n, "ip", "-j", "route"),
    ...
    if (ROOT / "storage.json").exists():
        ...
        result["physical_bytes"] = [
            (ROOT / f"disk{i}").stat().st_blocks * 512 for i in range(4)
        ]
    return result


def down():
    for n in ["proxy", "objects", "empty"]:
        stop(n)
    f = json.loads((ROOT / "storage.json").read_text())
    validate_storage(f)
    if os.path.ismount(f["mount"]):
        run("umount", f["mount"])
...
if __name__ == "__main__":
    ...
    try:
        if a.action == "init":
            if not a.confirm_disposable:
                raise RuntimeError("explicit disposable VM flag required")
            ...
            if a.action == "up":
                ...
                for n in [CLIENT, SERVER]:
                    ns(n, "ip", "link", "set", "wg0", "mtu", "1200")
            elif a.action == "stop-owner":
                stop("objects")
            elif a.action == "start-owner":
                workloads()
            elif a.action == "recover-storage":
                storage()
                workloads()
